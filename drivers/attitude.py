"""
姿态算解本脚本用于测试下位机由电子罗盘计算得到的偏航角与由姿态解算算法输出的偏航角的评估
    ...
    while True:
        ...
        if (counterForMagData == 3):
            ...
            if (dataIn.isdigit()):
                ...
                timeSequence1.append(
                    (timeNow - startTime).seconds + ((timeNow - startTime).microseconds) / 1000000)
        ...
        if (counter == 1):
            ...
            if sumUp == revData[21]:  # 校验和验证通过
                ...
                ValidShip.yaw = int.from_bytes(
                    Ship.yaw, byteorder='big', signed=False)
                ValidShip.pitch = int.from_bytes(
                    Ship.pitch, byteorder='big', signed=True)
                ValidShip.roll = int.from_bytes(
                    Ship.roll, byteorder='big', signed=True)
                ValidShip.lattitude = (
                    int.from_bytes(
                        Ship.lattitude,
                        byteorder='big',
                        signed=False) / 1e7)
                ValidShip.longtitude = (
                    int.from_bytes(
                        Ship.longtitude,
                        byteorder='big',
                        signed=False) / 1e7)
        ...
        if (timeSpan >= eval(timeSet)):
            ...
            for time in range(len(errorTimeSequence)):
                error.append(
                    yawReceivedFromMadgwick[time] -
                    yawReceivedFromCompass[time])
            # 画图
            plot1 = pl.plot(
                timeSequence1,
                yawReceivedFromCompass,
                'b',
                label='Computed from Compass')
            plot2 = pl.plot(
                timeSequence2,
                yawReceivedFromMadgwick,
                'g',
                label='Computed from Algorithm')
            ...
            pl.savefig(str("C:\\Users\\85234\\Desktop\\Madgwick_Eval\\" +
